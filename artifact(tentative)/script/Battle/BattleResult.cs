...
public class BattleResult : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        //結果表示後は結果表示テキストをスクロールしてみれるようにする
        if (Input.GetAxis("Vertical") != 0f)
        {
            resultText.transform.localPosition += new Vector3(0f, -Input.GetAxis("Vertical") * scrollValue, 0f);
        }
        //戦闘を抜け出すまでの待機時間を超えていない
        if (!isFinishResult)
        {
            return;
        }
        //SubmitやActionやFire1ボタンを押したらワールドマップに戻る
        if (Input.GetButtonDown("Submit") || Input.GetButtonDown("Action") || Input.GetButtonDown("Fire"))
        {
            if (won || ranAway)
            {
                GameObject.Find("SceneManager").GetComponent<LoadSceneManager>().MoveToStartScene();
            }
            else
            {
                GameObject.Find("SceneManager").GetComponent<LoadSceneManager>().MoveToStartScene();
    ...
    public IEnumerator DisplayVictoryResult(List<GameObject> allCharacterList, List<GameObject> allyCharacterInBattleList)
    {
        ...
        foreach (var characterObj in allyCharacterInBattleList)
        {
            ...
            //レベルアップ分のステータスアップを計算し反映する,乱数がそれぞれのステアップ確率より大きければそのステが上昇
            ...
            if (levelUpCount > 0)
            {
                resultText.text += character.GetCharacterName() + "は" + character.GetLevel() + "になった。\n";
                if (raisedHp > 0)
                {
    ...
    public IEnumerator DisplayDefeatResult()
    {
        yield return new WaitForSeconds(timeToDisplay);
        resultPanel.SetActive(true);
        resultText.text = "ユニティちゃんたちは全滅した。";
        isDisplayResult = true;
        yield return new WaitForSeconds(timeToDisplay);
        var finishText = resultPanel.transform.Find("FinishText");
        finishText.GetComponent<Text>().text = "スタート画面へ";
    ...
    public IEnumerator DisplayRanAwayResult()
    {
        ...
        yield return new WaitForSeconds(timeToDisplay);
        var finishText = resultPanel.transform.Find("FinishText");
        finishText.GetComponent<Text>().text = "スタート画面へ(Enter)";
        finishText.gameObject.SetActive(true);
        isFinishResult = true;
    }
}
