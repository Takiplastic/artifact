...
public class CharacterBattleScript : MonoBehaviour
{
   ...
   public enum BattleState
    ...
    private void Update()
    {
        ...
        Debug.Log(this.name+":"+battleState);
        ...
        if (battleState == BattleState.DirectAttack)
        {
            ...
            if (characterStatus as AllyStatus !=null)
            {
                battleState = BattleState.WaitingAttack;
                return;
            }
        ...
        else if(battleState == BattleState.WaitingAttack)
        {
            return;
        }
        else if (battleState == BattleState.WaitingStop)
        {
            Debug.Log("whilecheck");
            foreach(var child in targetObjList)
            {
                if (child.GetComponent<CharacterBattleScript>().isDead)
                {
                    targetObjList.Remove(child);
                }
            }
            foreach (var child in targetObjList)
            {
            ...
            Debug.Log("ターン終了");
            //速度減衰および衝突ダメージを定義したスクリプトの無効化
            foreach (var child in targetObjList)
            {
                child.GetComponent<enemy_velocity>().flag = false;
            }
            //ターゲットのリセット
            currentTarget = null;
            currentSkill = null;
    ...
    public void ChooseAttackOptions(BattleState selectOption, Skill skill = null, Item item = null)
    {
        ...
        battleState = selectOption;
        //アニメーションに移る前に技範囲を非表示
        this.gameObject.transform.Find("SkillAreaPos").GetChild(0).gameObject.SetActive(false);
        if (selectOption == BattleState.DirectAttack)
        {
            animator.SetTrigger("DirectAttack");
    ...
    IEnumerator DirectAttack()
    {
        ...
        { 
            ...
            {//味方の攻撃
                ...
                targetObjList[i].GetComponent<BattleEnemyStatus>().UpdateEnemyStatus(targetCharacterStatusList[i], BattleEnemyStatus.Status.HP, targetCharacterBattleScriptList[i].GetHp());
                //攻撃相手が死んでいたならtargetリストから除外
                if (targetCharacterBattleScriptList[i].isDead)
                {
                    targetObjList.Remove(targetObjList[i]);
                }
            }
            else
            {//敵の攻撃
        ...
        if (characterStatus as AllyStatus != null)
        {
            if(targetObjList.Count != 0)
            {
                ...
                StartCoroutine(ImpulseEnemy());
            }
            else
            {
                battleState = BattleState.WaitingStop;
            }
        }
    }
    ...
    //敵を吹っ飛ばす際に呼び出すメソッド
    public IEnumerator ImpulseEnemy()
    {
        Debug.Log("Impulse");
        animator.SetTrigger("BlowAttack");
        yield return new WaitForSeconds(0.5f);
        foreach(var child in targetObjList)
        {
            //速度減衰および衝突ダメージを定義したスクリプトを有効化
            child.GetComponent<enemy_velocity>().flag = true;
            //打ち出す力を設定
            int Force = 20;
            //打ち出す方向を設定
            Vector3 vector = (child.transform.position - this.transform.position).normalized;
            child.GetComponent<Rigidbody>().velocity = Force * vector;
            
            Debug.Log("Impulse:"+child.name+"を吹っ飛ばした");
            battleManager.ShowMessage(this.name + "の" + "ぶっ飛ばし");
        }
        battleState = BattleState.WaitingStop;
    }    
}
